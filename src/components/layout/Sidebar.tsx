...
export function Sidebar() {
  ...
  return (
    <>
      {/* ── Desktop Sidebar (lg+) ─────────────────────────────── */}
      <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-60 flex-col border-r border-white/8 bg-[#0a0f1e] z-40">
        {/* Logo */}
      ...
      {/* ── Mobile Top Bar (< lg) ─────────────────────────────── */}
      <header className="lg:hidden fixed top-0 left-0 right-0 h-14 bg-[#0a0f1e]/95 backdrop-blur-md border-b border-white/8 z-40 flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#6366f1] to-[#8b5cf6] flex items-center justify-center shadow-lg shadow-[#6366f1]/30">
            <Zap className="w-3.5 h-3.5 text-white" />
          </div>
          <span className="text-[#f8fafc] font-semibold text-sm tracking-tight">
            Flow<span className="gradient-text-brand">Split</span>
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button className="relative w-8 h-8 rounded-xl bg-white/5 border border-white/8 flex items-center justify-center text-[#475569] hover:text-[#94a3b8] transition-all">
            <Bell className="w-4 h-4" />
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#0a0f1e]/95 backdrop-blur-md border-t border-white/8 z-40 flex items-center">
        {navItems.map((item) => {
          const isActive =
            pathname === item.href ||
            (item.href !== "/dashboard" && pathname.startsWith(item.href));
          return (
            ...
            >
              <div className={cn(
                ...
                <item.icon className={cn("w-4 h-4", isActive ? "text-[#6366f1]" : "")} />
              </div>
